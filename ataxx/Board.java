...
class Board {
    ...
    void clear() {
        ...
        for (int i = 0; i < _board.length; i++) {
            if (i < EXTENDED_SIDE * 2 || i >= EXTENDED_SIDE * 9) {
                unrecordedSet(i, BLOCKED);
            } else if (i % 11 == 0 || i % 11 == 1
                    || i % 11 == 9 || i % 11 == 10) {
                unrecordedSet(i, BLOCKED);
            } else if (i == EXTENDED_SIDE * 9 - 3
                    || i == EXTENDED_SIDE * 2 + 2) {
                unrecordedSet(i, BLUE);
            } else if (i == EXTENDED_SIDE * 3 - 3
                    || i == EXTENDED_SIDE * 8 + 2) {
                unrecordedSet(i, RED);
            } else {
    ...
    PieceColor getWinner() {
        ...
        if (numPieces(_whoseMove) == 0) {
            _winner = opponent;
        } else if (numPieces(opponent) == 0) {
            _winner = _whoseMove;
        } else if ((!canMove(_whoseMove) && !canMove(opponent))
                || numJumps() >= JUMP_LIMIT) {
            if (numPieces(_whoseMove) > numPieces(opponent)) {
                _winner = _whoseMove;
            } else if (numPieces(_whoseMove) < numPieces(opponent)) {
                _winner = opponent;
            } else {
                _winner = EMPTY;
            }
    ...
    boolean legalMove(Move move) {
        ...
        if (move == null) {
            ...
            return (_board[move.fromIndex()] == whoseMove());
        }
            ...
            if (get(i) == who) {
                for (int dc = -2; dc < 3; dc++) {
                    for (int dr = -2; dr < 3; dr++) {
                        if (_board[neighbor(i, dc, dr)] == EMPTY) {
                            return true;
                        }
                    }
                }
    ...
    void makeMove(Move move) {
        ...
        if (move.isExtend()) {
            _numJumps = 0;
            _numPieces[_whoseMove.ordinal()] += 1;

            set(move.toIndex(), _whoseMove);
            for (int dc = -1; dc < 2; dc++) {
                for (int dr = -1; dr < 2; dr++) {
                    if (_board[neighbor(move.toIndex(), dc, dr)] == opponent) {
                        _numPieces[_whoseMove.ordinal()] += 1;
                        _numPieces[opponent.ordinal()] -= 1;
                        set(neighbor(move.toIndex(), dc, dr), whoseMove());
                    }
                }
            }
            ...
            for (int dc = -1; dc < 2; dc++) {
                for (int dr = -1; dr < 2; dr++) {
                    if (_board[neighbor(move.toIndex(), dc, dr)] == opponent) {
                        _numPieces[_whoseMove.ordinal()] += 1;
                        _numPieces[opponent.ordinal()] -= 1;
                        set(neighbor(move.toIndex(), dc, dr), _whoseMove);
                    }
                }
            }
    ...
    boolean legalBlock(char c, char r) {
        return _board[index(c, r)] == EMPTY && _allMoves.isEmpty();
    ...
    void setBlock(char c, char r) {
        ...
        char mirrorRow = (char) ('1' + '7' - r);
        char mirrorCol = (char) ('a' + 'g' - c);

        unrecordedSet(c, r, BLOCKED);
        unrecordedSet(mirrorCol, mirrorRow, BLOCKED);
        unrecordedSet(c, mirrorRow, BLOCKED);
        unrecordedSet(mirrorCol, r, BLOCKED);
